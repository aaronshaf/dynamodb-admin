 ...
 */
function doSearch(docClient, tableName, scanParams, limit,
                  startKey, progress, readOperation = 'scan') {
  ...
  const readMethod = {
    scan: (...args) => docClient.send(new ScanCommand(...args)),
    query: (...args) => docClient.send(new QueryCommand(...args))
  }[readOperation]
