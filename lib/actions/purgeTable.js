...
const findPrimaryKeys = context => {
  ...
  return context.client.describeTable(params)
...
const findAllElements = context => {
  ...
  return doSearch(context.client, context.tableName, scanParams)
    .then(items => {
      context.items = items
      return context
    })
...
const deleteAllElements = context => {
  ...
  for (const item of context.items) {
    ...
    if (counter % MAX_OPERATIONS === 0) {
      deleteRequests.push(context.client.batchWriteItem(params))
 ...
 * @param client the AWS dynamodb service that holds the connection
 * @returns {Promise<any[] | never>} concatenation of all delete request promises
 */
const purgeTable = (tableName, client) => {
  const context = {
    tableName,
    client
  }
