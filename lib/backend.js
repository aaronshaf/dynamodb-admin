...
exports.createServer = (dynamodb, docClient) => {
  ...
  app.post('/config/cookie', bodyParser.json({}), (req, res) => {
    const { theme } = req.body
    if (!theme) { return res.send({ ok: false }) }
  ...
  app.get('/tables/:TableName/get', asyncMiddleware((req, res) => {
    ...
    if (req.query.hash) {
      if (req.query.range) {
        return res.redirect(
          `/tables/${encodeURIComponent(TableName)}/items/${encodeURIComponent(req.query.hash)},${encodeURIComponent(req.query.range)}`
        )
      } else {
        return res.redirect(`/tables/${encodeURIComponent(TableName)}/items/${encodeURIComponent(req.query.hash)}`)
      }
    }
