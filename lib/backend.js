...
function loadDynamoConfig(env, local_config) {
  ...
  const dummy_creds = local_config ?  {}: {credentials: {
    accessKeyId: 'key',
    secretAccessKey: 'secret',
    region: 'us-east-1',
  }}
  dynamoConfig = {local_config, ...dynamoConfig, ...dummy_creds}

  loadDynamoEndpoint(env, dynamoConfig)
  return dynamoConfig
}

const createAwsConfig = (local_config) => {
  const env = process.env
  const dynamoConfig = loadDynamoConfig(env, local_config)
...
exports.createServer = (dynamodbClient, docClient, local_config, expressInstance = express()) => {
  ...
  if (!dynamodbClient || !docClient) {
    if (!dynamodbClient) {
      dynamodbClient = new DynamoDBClient(createAwsConfig(local_config))
    }
