...
function loadLocalConfiguration(env) {
    if (!AWS.config.credentials) {
        let localFileCredentials = new AWS.Credentials({
            accessKeyId: env.AWS_ACCESS_KEY_ID || 'key',
            secretAccessKey: 'secret',
            sessionToken: 'session'
        });

        AWS.config.update({credentials: localFileCredentials});
    }
...
exports.createServer = (dynamodb, docClient) => {
  ...
  if (!dynamodb || !docClient) {
    if (!dynamodb) {
      dynamodb = new AWS.DynamoDB(createAwsConfig())
    }
