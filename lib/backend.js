...
exports.createServer = (dynamodb, docClient) => {
  ...
  app.post(
    ...
    asyncMiddleware((req, res) => {
      ...
      if (localSecondaryIndexes === undefined || localSecondaryIndexes.length
                === 0) {
      ...
      if (req.query.range) {
        return res.redirect(
          `/tables/${encodeURIComponent(TableName)}/items/${
            encodeURIComponent(req.query.hash)},${encodeURIComponent(
            req.query.range)}`
        )
      } else {
        return res.redirect(`/tables/${
          encodeURIComponent(TableName)}/items/${encodeURIComponent(
          req.query.hash)}`)
      }
    }
  ...
  const getPage = (docClient, keySchema, TableName, scanParams, pageSize,
    ...
    function onNewItems(items, lastStartKey) {
      for (let i = 0; i < items.length && pageItems.length < pageSize + 1;
        i++) {
  ...
  app.get('/tables/:TableName/items', asyncMiddleware((req, res) => {
    ...
    return describeTable({ TableName })
      .then(description => {
        ...
        if (req.query.queryableSelection && req.query.queryableSelection
                    !== 'table') {
...
function isAttributeNotAlreadyCreated(attributeDefinitions, attributeName) {
  return !attributeDefinitions
    .find(attributeDefinition => attributeDefinition.AttributeName
            === attributeName)
}
